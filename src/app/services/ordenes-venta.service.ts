...
export class OrdenesVentaService {
 //---------------------------------------------//
  ...
  constructor(public http: HttpClient,  public uiService: UiService, public loadingController: LoadingController, public router: Router) { }
  public async load() {
    debugger
      ...
      try {
        ...
        if (control.codigo == "OK"){
            if(response.datos.ordenes.length > 0) {
              response.datos.ordenes.sort((a:any, b:any) => new Date(a.fechaActualizado) <= new Date(b.fechaActualizado) ? 1 : -1);
...
public crearOrden(aOrdenar: any) {
  ...
  return new Promise(async (resolve, reject) => {
    try {
        ...
        headers: new HttpHeaders({
          token: this.usuarioActual.token.hashId,
             codigoCereal: aOrdenar.codigoCereal,
                  cosecha: aOrdenar.mercado == 5 ? aOrdenar.cosecha.substr(0,2) + aOrdenar.cosecha.substr(3,2) : aOrdenar.cosecha.toString(),
                  precioBase: aOrdenar.precioBase,
                  toneladas: aOrdenar.toneladas * 1000,
                  idMoneda: aOrdenar.moneda,
                  idMercado: aOrdenar.mercado,
                  observaciones: ".",
                  fechaCobro: aOrdenar.mercado == 6 ? aOrdenar.fechaCobro.toLocaleDateString('en-GB', { timeZone: "UTC" }) : ""
        }),
      };
      ...
      this.ordenesVentaPendientes = [];
      this.ordenesVentaTerminadas = [];
      this.http.post(url,  bodyOptions,  httpOptions ).subscribe((response : any)   => {
        ...
        if (control.codigo == "OK"){
          ...
          if(control.codigo == "OK"){
            ...
            this.uiService.presentAlertInfo("Orden almacenada, Aguarde a que un operador de la "+Configuraciones.razonSocialNombreCorto+" apruebe su orden.")


          };

          } else {
            this.uiService.presentAlertInfo("Orden fallida, Su orden no pudo ser procesada, inténtelo más tarde.")
